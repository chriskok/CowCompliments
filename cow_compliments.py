...
def write_compliment():
    chosen_compliment = COMPLIMENT_ARRAY[random.randint(0, len(COMPLIMENT_ARRAY) - 1)]
    while (len(chosen_compliment) > 85):
        chosen_compliment = COMPLIMENT_ARRAY[random.randint(0, len(COMPLIMENT_ARRAY) - 1)]

    pyautogui.typewrite(chosen_compliment, interval=0.10)
...
def click_coordinates(predictions):
    ...
    pyautogui.click()
    # pyautogui.click(x=x_coord, y=y_coord, button='left', duration = 0.1)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        result = click_coordinates(results)
        if (result == 1): time.sleep(10)
